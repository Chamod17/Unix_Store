      ...
      <div class="col-xl-8">
          ...
          <div class="tab-pane fade" id="list-cart" role="tabpanel" aria-labelledby="list-cart-list">
              <div style="width: 70%;float:left;padding:25px" id="list-cart-inside">
                          <?php 
                            $sql = "SELECT products.Image, products.Price, products.Name, products.ID, products.Quantity AS Quantity2 , cart.Quantity, cart.ID AS cart FROM cart INNER JOIN products ON cart.ProductID = products.ID where cart.CustomerEmail = '$_SESSION[email]'
                                    UNION
                                    SELECT custompc.Image, custompc.Price, custompc.Name, custompc.ID, cart.Quantity AS Quantity2, cart.Quantity, cart.ID AS cart FROM cart INNER JOIN custompc ON cart.ProductID = custompc.ID where cart.CustomerEmail = '$_SESSION[email]';
                                    ";

                              $result = mysqli_query($link, $sql);
                              //echo "why".mysqli_error($link);
                              if(mysqli_num_rows($result)<1){
                                echo '<img src="assets/empty_cart.png" width="100%" alt="">';
                              }
                        ...
                        <div class="row border shadow p-3 mb-3 bg-white rounded" >
                            ...
                            <div class="col-6">
                                <p class="card-text"><?php echo $row['Name']?></p><h6>Quantity 
                                        <label id="demo<?php echo $row['cart']?>"><?php if($row['Quantity'] > $row['Quantity2']){echo $row['Quantity2'];}else{echo $row['Quantity'];} ?></label>
                                        <input type="range" oninput="Slider(this)" onchange="cartUpdate(this)" disabled min="1" max="<?php if($row['Name']=="Custom PC"){echo "4";}else{echo $row['Quantity2'];} ?>" value="<?php if($row['Quantity'] > $row['Quantity2']){echo $row['Quantity2'];}else{echo $row['Quantity'];} ?>" id="<?php echo $row['cart']?>" name="<?php echo $row['ID']?>">
                        ...
                        <script>
                          //===================================================================================
                            function Slider(slid) {
                              console.log("demo"+slid.id);
                              document.getElementById("demo"+slid.id).innerHTML = slid.value;
                            }
            ...
            <div class="tab-pane fade" id="list-shipping" role="tabpanel" aria-labelledby="list-shipping-list">
              <br><h1>Update Information</h1><br>
                  ...
                  <script>
                        const myForm = document.getElementById('userUpdateInfo');
            ...
            <!-- ======================================================================================================================================================== -->            
              ...
              <form id="changeName" >
                    ...
                    <input type="text" hidden name="task" value="changeName">
                    <button type="submit" class="btn btn-primary">Update</button>
              </form>
                  <script>
                        const myFormName = document.getElementById('changeName');

                        myFormName.addEventListener('submit', function(e){
                            e.preventDefault();
            ...
            <!-- ======================================================================================================================================================== -->
              ...
              <script>
                        ...
                        changePass.addEventListener('submit', function(e){
                            ...
                            fetch('database/change_password.php', {
                                method:"POST",
                                body: formData
                            }).then(function (response) {
                                return response.text();
...
<hr>
            <!-- ======================================================================================================================================================== -->
            <h4>Update Propic</h4>
              <form id="changePropic" enctype="multipart/form-data"> 
                  <div class="form-row">
                    <div class="form-group col-md-3">
                        <input type="file" class="form-control-file" name="image" onchange="readURL(this);">
                    </div>
                    <div class="form-group col-md-3">
                        <img id="preview" src="<?php echo $userRow["Propic"]?>" alt="your image" style="width: 200px;">
                    </div>
                    <script>
              ...
              <script>
                      const myFormPropic = document.getElementById('changePropic');

                      myFormPropic.addEventListener('submit', function(e){
                          e.preventDefault();

                          const formData = new FormData(this);

                          fetch('database/update_propic.php', {
                              method:"POST",
                              body: formData
                          }).then(function (response) {
                              return response.text();
